...
describe("ProtoCoin test", function () {
  ...
  it("Should NOT transfer from (balance)", async function () {
    ...
    const instance = protoCoin.connect(otherAccount);
    await expect(instance.transferFrom(otherAccount.address, otherAccount, 1n)).to.be.revertedWith("Insufficient balance");
  });
  ...
  it("Should NOT transfer from (allowance)", async function () {
    ...
    const instance = protoCoin.connect(otherAccount);
    await expect(instance.transferFrom(owner.address, otherAccount, 1n)).to.be.revertedWith("Insufficient allowance");
  });
});
